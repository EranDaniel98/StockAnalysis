"""Momentum-oscillator merge A/B sweep.

Two modes on the swing_trading + themes harness:

  separate : legacy — RSI and Stochastic both contribute independent
             composite scores and emit independent signals.
  merged   : single ``MomOsc`` composite slot (averaged) and one signal
             emitted only when RSI and Stoch agree. Cuts the double-
             counting in both the technical composite and the engine's
             signal-consensus adjustment.

Hypothesis (audit): the double-count is real but small — net effect
will land inside the same noise band as prior sweeps (~+/-0.5 OOS
Sharpe). Worth measuring because the cost is one signal slot and the
upside is a cleaner consensus tally.
"""
...
def _set_merge(config: Config, merged: bool) -> None:
    rm = config.settings.setdefault("risk_management", {})
    rm.setdefault("momentum_oscillator", {})["merge_rsi_stoch"] = merged


def _summarize(mode: str, merged: bool, result: dict[str, Any]) -> dict[str, Any]:
    full = result["full"]
...
def _print_table(rows: list[dict[str, Any]], strategy_name: str, universe_label: str) -> None:
    table = Table(
        title=f"Oscillator merge A/B — {strategy_name} on {universe_label}",
        show_lines=False,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="A/B sweep on RSI+Stoch merge.")
    ...
    parser.add_argument(
        "--save",
        default="data/sweep_oscillator_merge.json",
    )
    ...
    console.print(f"\n[bold cyan]Oscillator-merge A/B sweep[/bold cyan]")
    ...
    for mode, merged in MODES:
        _set_merge(config, merged)
        ...
        summary = _summarize(mode, merged, result)
        console.print(
