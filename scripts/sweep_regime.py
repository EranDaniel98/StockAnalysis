"""Regime-gate A/B sweep.

Re-runs the recommended-live swing_trading backtest with three gate
modes — ``off``, ``skip_bear``, ``skip_bear_and_chop`` — on the same
universe, window, and price/fundamentals snapshot. Prints a comparison
table and saves the raw rows to JSON.

Why this is a separate script (not a flag on parameter_sweep):
The existing parameter_sweep harness varies ``BacktestConfig`` fields
via ``dataclasses.replace``. The regime gate lives on the ``Config``
object (``risk_management.regime_filter``), not on ``BacktestConfig``,
so we mutate it directly between runs. Data fetches happen once and
get reused across all three runs.

Anchor: matches the live-recommended config from
``project_sweep_results.md`` (swing_trading + min_score=50 +
atr_stop_mult=2.0 + themes universe + 3y window).
"""
...
GATE_MODES = ["off", "skip_bear", "skip_bear_and_chop"]


def _set_regime_mode(config: Config, mode: str) -> None:
    """Mutate the in-memory config so the engine sees the new gate
    setting on its next run. We do NOT touch settings.yaml on disk —
    the script is non-destructive."""
    rm = config.settings.setdefault("risk_management", {})
    rf = rm.setdefault("regime_filter", {})
    rf["enabled"] = mode != "off"
    rf["mode"] = mode
    rf.setdefault("sma_period", 200)
    rf.setdefault("vix_low", 20.0)
    rf.setdefault("vix_high", 25.0)


def _summarize(mode: str, result: dict[str, Any]) -> dict[str, Any]:
    """Pull the headline numbers a human compares between modes.

    OOS Sharpe is the primary metric (matches the existing sweep code).
    Trade count and entries_blocked tell us *what* the gate did; max
    drawdown tells us whether the protection was worth the lost upside.
    """
    full = result["full"]
    oos = result["out_of_sample"]
    gate = result.get("regime_gate", {})
    return {
        "mode": mode,
        ...
        "entries_blocked": gate.get("entries_blocked", 0),
        "mondays_blocked": gate.get("mondays_blocked", 0),
    }


def _print_table(rows: list[dict[str, Any]], strategy_name: str, universe_label: str) -> None:
    table = Table(
        title=f"Regime gate A/B — {strategy_name} on {universe_label}",
        show_lines=False,
    )
    ...
    for r in rows:
        table.add_row(
            ...
            f"{r['max_dd_pct']:.2f}",
            f"{r['win_rate_pct']:.1f}",
            f"{r['entries_blocked']} ({r['mondays_blocked']}M)",
        )
    console.print(table)


def main() -> int:
    parser = argparse.ArgumentParser(description="A/B sweep on the regime gate.")
    ...
    parser.add_argument(
        "--save",
        default="data/sweep_regime.json",
    ...
    # Need 200d of pre-start history for SMA200 + the 1y SPY+VIX history the
    # classifier prefers, so pull years+2 with a 5y floor — matches cmd_backtest.
    fetch_period_years = max(args.years + 2, 5)
    fetch_period = f"{int(fetch_period_years)}y"

    console.print(f"\n[bold cyan]Regime A/B sweep[/bold cyan]")
    ...
    for mode in GATE_MODES:
        ...
        console.print(
            ...
            f"trades {summary['n_trades']}, blocked {summary['entries_blocked']}\n"
        )
        rows.append(summary)
